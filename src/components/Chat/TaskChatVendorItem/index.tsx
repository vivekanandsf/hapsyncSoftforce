...
const TaskChatVendorItem = props => {
   const { taskData, vendor, chatName } = props;
   ...
   const groupName = `task-group-${taskData?.id}_vendor-${vendor?.userId}`;

   let guestIds = []
   if (assignees) {
      guestIds = assignees?.map((assignee) => `${assignee?.userId}_user`);
      //adding vendor to assignees
      guestIds.push(`${vendor?.userId}_user`)
   }
   ...
   return <Pressable
      ...
      >
         <View>
            ...
            >
               Group Chat {chatName}{/* #{eventData?.id} */}
            </Text>
            {renderLastMessage()}
         </View>
         {/* <View style={{
...
export default TaskChatVendorItem